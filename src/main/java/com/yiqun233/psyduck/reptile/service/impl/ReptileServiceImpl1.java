...
@Slf4j
@Service
public class ReptileServiceImpl1 implements ReptileService1 {
    ...
    public void getChildAddress() throws IOException {
        ...
        for (int i = 2020; i < 2024; i++) {
            for (int j = 1; j < 7; j++) {
                String url = "http://www.cjxb.ac.cn/cn/article/" + i + "/" + j;
                ...
                for (Element element : select) {
                    ...
                    String title = element.selectFirst("div.article-list-title").text();
                    try {
                        getLiteratureContent(contentUrl, title);
                    } catch (Exception e) {
